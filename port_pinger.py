...
from db_helper import insert_ping_sweep_result, insert_network_info

class PPPPP_GUI:
      def __init__(self, master):
        self.ping_sweep_active = False
      ...
      def configure_gui(self):
        # Set background to blue theme
        self.master.configure(bg='  ')

      def init_widgets(self):
        # Initialize your widgets here
        # Make sure to add widgets to 'self.master' instead of 'window'

        # Example widget in blue theme
        label = tk.Label(self.master, text="Port Pinger", bg='#0A74DA', fg='white')
        label.pack()
        
      def create_widgets(self):
        # Initialize widgets here, ensuring they're added to 'self.master'
        # Adjust all widgets to match the blue theme

        # Network Information Display
        self.LFRM_Current_Network = tk.LabelFrame(self.master, text="Current Network", bg='#0A74DA', fg='white', font=('Arial', 10, 'bold'))
        self.LFRM_Current_Network.pack(fill='both', expand=True, padx=10, pady=5)
        self.BTN_Stop_Ping = tk.Button(self.LFRM_Ping_Sweeper, text="STOP", bg='#FF0000', fg='white', command=self.stop_ping_sweep)
        self.BTN_Stop_Ping.pack(side='right', padx=10)
        self.TXT_Current_Network = tk.Text(self.LFRM_Current_Network, bg='#FFFFFF', fg='black', height=10)
        self.TXT_Current_Network.pack(fill='both', expand=True)
        self.get_current_network()

        # Ping Sweep Frame
        self.LFRM_Ping_Sweeper = tk.LabelFrame(self.master, text="Ping Sweeper", bg='#0A74DA', fg='white', font=('Arial', 10, 'bold'))
        self.LFRM_Ping_Sweeper.pack(fill='both', expand=True, padx=10, pady=5)

        # Add Ping Sweeper Widgets Here...

        # Port Scanner Frame
        self.LFRM_Port_Scanner = tk.LabelFrame(self.master, text="Port Scanner", bg='#0A74DA', fg='white', font=('Arial', 10, 'bold'))
        self.LFRM_Port_Scanner.pack(fill='both', expand=True, padx=10, pady=5)
        self.BTN_Stop_Scan = tk.Button(self.LFRM_Port_Scanner, text="STOP", bg='#FF0000', fg='white', command=self.stop_port_scan)
        self.BTN_Stop_Scan.pack(side='right', padx=10)
...
#------Inline Functions------
      def Get_Current_Network(self): 
          ...
          print("\n" + MESSAGE);  
          insert_network_info(IP_Address, Subnet_Mask, Default_Gateway, DNS_Servers)      
      ...
      def Ping_Sweep_1(self):
          ...
          self.TXT_Main_Output.delete(1.0,"end");
          self.TXT_Main_Output.insert("0.0", Txt_Box_Contents + MESSAGE); 
          live_hosts = "List or summary of live hosts"
          insert_ping_sweep_result(Net_IP, Start_Host, End_Host, live_hosts)
    ...
    #---Constructor----------------------------------------------------------------------------     
      def __init__(self, master=None):
          ...
          def Help_Menu_HELP_Handler(): 
              MESSAGE = "Welcome to the Narayn Port Pinger. This project is";
          ...
          def Help_Menu_ABOUT_Handler(): 
              MESSAGE = "Narayan Port Pinger 1.0\nCarly Salali Germany - 2022";
...
#---A. Frame: Main Window -------------------------------------------------------------
          ...
          self.FRM_Main_Window.configure(height=520, width=1100, borderwidth=3, relief="flat", background="#00069b");
          self.FRM_Main_Window.place(anchor="nw", height=520, width=1100, x=0, y=0);

          self.LAB_Title = tk.Label(self.FRM_Main_Window);
          self.LAB_Title.configure(background="#00069b",foreground="#FFFFFF", font="{Barlow Condensed} 13 {}", text="Narayan Port Pinger");
          ...
          self.LFRM_Ping_Sweeper.configure(height=200, width=377, borderwidth=3, relief="sunken", background="#00069b", text="Ping Sweeper");
          ...
          self.RB_Activate_Ping_Sweeper.configure(height=25, width=190, background="#00069b", font="{Arial} 12 {}", text="Activate Ping Sweeper");
          ...
          self.LAB_Network.configure(background="#00069b", borderwidth=0, relief="flat", font="{Arial} 12 {}", anchor="nw", text="Network:");
          self.LAB_Network.place(anchor="w", x=22, y=57);

          self.LAB_Start_Host = tk.Label(self.LFRM_Ping_Sweeper);
          self.LAB_Start_Host.configure(background="#00069b", borderwidth=0, relief="flat", font="{Arial} 12 {}", anchor="nw", text="Start Host:");
          self.LAB_Start_Host.place(anchor="w", x=22, y=92);

          self.LAB_End_Host = tk.Label(self.LFRM_Ping_Sweeper);
          self.LAB_End_Host.configure(background="#00069b", borderwidth=0, relief="flat", font="{Arial} 12 {}", anchor="nw", text="End Host:");
...
#---D. Frame: Port Scanner -------------------------------------------------------------            
          ...
          self.LFRM_Port_Scanner.configure(height=200, width=377, borderwidth=3, relief="sunken", background="#00069b", text="Port Scanner");
          ...
          self.RB_Activate_Port_Scanner.configure(height=25, width=190, background="#00069b", font="{Arial} 12 {}", text="Activate Port Scanner");
          self.RB_Activate_Port_Scanner.place(anchor="nw", height=25, width=190, x=18, y=10);

          self.LAB_Host = tk.Label(self.LFRM_Port_Scanner);
          self.LAB_Host.configure(background="#00069b", borderwidth=0, relief="flat", font="{Arial} 12 {}", anchor="nw", text="Host:");
          self.LAB_Host.place(anchor="nw", x=20, y=45);

          self.LAB_Start_Port = tk.Label(self.LFRM_Port_Scanner);
          self.LAB_Start_Port.configure(background="#00069b", borderwidth=0, relief="flat", font="{Arial} 12 {}", anchor="nw", text="Start Port:");
          self.LAB_Start_Port.place(anchor="nw", x=20, y=80);

          self.LAB_End_Port = tk.Label(self.LFRM_Port_Scanner);
          self.LAB_End_Port.configure(background="#00069b", borderwidth=0, relief="flat", font="{Arial} 12 {}", anchor="nw", text="End Port:" );
...
#---E. Frame: Main Output -------------------------------------------------------------
          ...
          self.LFRM_Main_Output.configure(height=480, width=700, borderwidth=3, relief="sunken", background="#00069b", text="Main Output");
...
#---F. Frame: Current Network  -------------------------------------------------------------
          ...
          self.LFRM_Current_Network.configure(height=95, width=377, borderwidth=3, relief="sunken", background="#00069b", text="Current Network");
