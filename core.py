import itertools
import time
from socket import AF_INET, SOCK_RAW, socket
...
class Decoder:
    ...
    def execute(self):
        with socket(AF_INET, SOCK_RAW, 1) as sock: # Use AF_INET and SOCK_RAW for Windows
            self._bind_interface(sock)
            for self.packet_num in itertools.count(1):
                self.frame_length = len(frame := sock.recv(65535))
